...
import { Checkmark, chunkLoaded, firstLetterCapital, rotateCoords } from "./Utils.js"
...
class Room {
    ...
    set checkmarkState(val) {
        ...
        let oldState = this.checkmarkState;
        let ooldState = this._checkmarkState;
        let playersC = this.getPlayersInRoom()
        sleep(1100, () => {
            if (oldState !== val && val !== Checkmark.GRAY && val !== Checkmark.NONE) {
                this.addEvent(RoomEvents.CHECKMARK_STATE_CHANGE, oldState, val)
                ...
                if (newlyCheckmarked && this.type !== Room.FAIRY && this.type !== Room.SPAWN) {
                    let players = playersC.length > 0 ? playersC : this.getPlayersInRoom();
    ...
    getGlobalCoords(relativeX, relativeY, relativeZ) {
        let x, y, z;
    
        // Apply inverse rotation
        switch (this.rotation) {
            case 2:
                x = this.minX + this.width - relativeZ;
                y = relativeY;
                z = this.minY + relativeX;
                break;
            case 3:
                x = this.minX + this.width - relativeX;
                y = relativeY;
                z = this.minY + this.height - relativeZ;
                break;
            case 0:
                x = this.minX + relativeZ;
                y = relativeY;
                z = this.minY + this.height - relativeX;
                break;
            case 1:
            default:
                x = this.minX + relativeX;
                y = relativeY;
                z = this.minY + relativeZ;
                break;
        }
