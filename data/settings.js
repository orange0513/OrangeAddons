...
export default class settings {
    ...
    constructor(main) {
        this.main = main;
        this.config = new DefaultConfig("OrangeAddons", "/data/settings.json");
        this.initConfig();
        this.settingObj = new Settings("OrangeAddons", this.config, "./data/ColorScheme.json", "&6&lOrangeAddons &e" + this.main.version);
        this.settingObj.AmaterasuGui.descriptionElement.textWrap.enabled = false;
        this.settingObj.AmaterasuGui.descriptionElement.textWrap.linesLimit = 7;
        this.settingObj.AmaterasuGui.descriptionElement.textWrap.wrapHeight = 7;
        this.settingObj.apply();
        this.values = this.settingObj.settings;
    }

    /**
     * @description Initializes the config for the module.
     * @returns {void}
     * @private
     */
    initConfig() {
        ...
        .addSwitch({
            category: "Misc",
            configName: "wttwmo",
            title: "&9Who Tried To Warp Me Out!",
            description: "&eWhen someone partys you, this feature will tell you what guild they are in, and whos also inside of that guild thats in your lobby & their SB Level + Profile Age",
            subcategory: "",
            value: false
        })
        .addSwitch({
    ...
    */
    openSettingsMenu() {
        let textArray = [...this.main.socket.infoMsg];

            let extraLines = textArray.length - 7;
            if (extraLines > 0)
            this.settingObj.AmaterasuGui.descriptionElement.textWrap.removeLines = (extraLines * -0.9);
        ...
        this.config
            ...
            this.config._makeConfig();
        this.settingObj.config = this.config.config;
        this.settingObj.apply();

        this.settingObj.openGui();
    }

    /**
     * @param {string} name
     * @param {any} value
     * @description Sets the value of a setting in the settings.json file.
     * @returns {void}
    */
    setValue(name, value) {
        try {
            const currentSettings = this.main.FileUtils.readJSON("/data/settings.json");
            let category = null;
            for (let cat of currentSettings) {
                if (category) break;
            ...
            if (category)
                this.settingObj.setConfigValue(category, name, value); 
        } catch (e) {
        }
    }
